...
main_application.iconbitmap(r'icon/SC Media Player.ico')
...
music_player_icon = PhotoImage(file = 'icon/SC Media Player Resized.png') # In About Us Section
...
def current_time_length(t):
    global pause_var, current_time, currentformat
    current_time = 0
    try:
...
def aboutus_func():
    ...
    text2 = ttk.Label(about_us_dialog, text = 'SC Music Player is a free open source platform for \nmusic made by Shubham Choudhary\n\nSC Music Player can read almost all music files and \nformats\n\nFor any queries contact - shubhcy99@gmail.com', font = ('Times New Roman', 12))
    text2.pack(side = TOP)
...
def open_folder(event = None):
    global playlist_box, playlist_list
